...
В summary для SVAR моделей есть ошибка
Нужно достать код функции 
```{r}
MSBVAR:::summary.BSVAR
```
Добавить в него первой строчкой 
```{r}
p <- dim(object$ar.coefs)[3]
```
И сохранить под видом какой-нибудь функции
...
Автор не ответил на письмо об ошибке, похоже нужно выкладывать пакет в открытый доступ с исправленной ошибкой самостоятельно. А студентам можно предложить дополнить его алгоритмами из MBR пакета.
