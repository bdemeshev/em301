...
Теперь вспоминаем, что \(\hat{y} = X\hat{\beta} \), \(\hat{\epsilon} = y - \hat{y} \), а \(\hat{\epsilon}'\hat{\epsilon} = RSS\), откуда:
...
* \(Var(y) = \sigma^2 I\ \)
* \(\mathbb{E}(y)\mathbb{E}(y') = X\beta \beta'X' \), т.к. \(y = X\beta + \epsilon, \mathbb{E}(y) = \mathbb{E}(X\beta) + \mathbb{E}(\epsilon) = X\beta \)
...
Наконец-то мы посчитали $RSS$. Теперь ясно, что оценка дисперсии параметра $\beta$ методом максимального правдоподобия --- смещенная!
...
2. Посчитать руками $H$, $I-H$, $rk(H)$, $rk(I-H)$, $tr(H)$, $tr(I-H)$, найти закон распределения $\dfrac{\epsilon'H\epsilon}{\sigma^2}$, $\dfrac{\epsilon'(I-H)\epsilon}{\sigma^2}$,  $\dfrac{y'(I-H)y}{\sigma^2}$ и $\dfrac{RSS}{\sigma^2}$ если известно, что 

\[
X = \left( \begin{matrix} 1 & 0.1 \\ 1 & 0.2 \\ 1 & 0.3 \end{matrix} \right), \quad y = X\beta + \epsilon, \quad \epsilon \sim \mathbb{N}(0, \sigma^2 I)
\]
