...
В регрессии с константой сумма квадратов остатков равна $\Sexpr{RSS}$, а число наблюдений равно $\Sexpr{n_obs}$. Точечная оценка дисперсии случайной составляющей равна
...
Гипотеза о том, что одновременно $\beta_1 + \beta_2 =1$  и $\beta_3=0$ в множественной линейной регрессии построенной по $n$ наблюдениям  проверяется с помощью  статистики, имеющей распределение
\begin{multicols}{3} % располагаем ответы в 3 колонки
\begin{choices} % опция [o] не рандомизирует порядок ответов
       \correctchoice{$F$}
       \wrongchoice{$t_n$}
