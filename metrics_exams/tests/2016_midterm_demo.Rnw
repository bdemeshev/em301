<<"ci_block_midterm", echo=FALSE, include=FALSE>>=
k_all <- c(5, 6, 7, 8, 9)
...
В множественной регрессии с константой матрица $X$ имеет размер $\Sexpr{n} \times \Sexpr{k}$, $TSS=\Sexpr{TSS}$, $ESS=\Sexpr{ESS}$. Несмещённая оценка для дисперсии случайных ошибок модели равна
\begin{multicols}{3} % располагаем ответы в 3 колонки
\begin{choices} % опция [o] не рандомизирует порядок ответов
       \correctchoice{$\Sexpr{hs2}$}
       \wrongchoice{$\Sexpr{wrong0}$}
       \wrongchoice{$\Sexpr{hs2 + 10}$}
       \wrongchoice{$\sqrt{\Sexpr{hs2 + 10}}$}
       \wrongchoice{$\sqrt{\Sexpr{hs2 + 20}}$}
       \wrongchoice{$\Sexpr{hs2 + 20}$}
    \end{choices}
   \end{multicols}
\end{questionmult}
}
...
\element{combat_kr_01_16}{ % в фигурных скобках название группы вопросов
 %\AMCnoCompleteMulti
  \begin{questionmult}{pr_01} % тип вопроса (questionmult — множественный выбор) и в фигурных — номер вопроса
    Если $\E(X) = \Sexpr{var_no + 3}$, $\E(Y) = \Sexpr{var_no + 2}$, $\Var(X) = 6$, $\Var(Y) = 7$, $\Cov(X,Y) = -1$, то $\Cov(\Sexpr{var_no} - X + 2Y, \Sexpr{var_no}X)$  равна
 \begin{multicols}{3} % располагаем ответы в 3 колонки
   \begin{choices} % опция [o] не рандомизирует порядок ответов
      \correctchoice{\Sexpr{var_no * (-8)}}
      \wrongchoice{\Sexpr{var_no * (8)}}
      \wrongchoice{\Sexpr{var_no * (4)}}
      \wrongchoice{\Sexpr{var_no * (-4)}}
      \wrongchoice{\Sexpr{var_no * (-8 + var_no)}}
      \wrongchoice{\Sexpr{var_no * (-8 - var_no)}}
   \end{choices}
  \end{multicols}
  \end{questionmult}
...
\element{combat_kr_01_16}{ % в фигурных скобках название группы вопросов
 %\AMCnoCompleteMulti
  \begin{questionmult}{pr_03} % тип вопроса (questionmult — множественный выбор) и в фигурных — номер вопроса
    Предпосылки теоремы Гаусса-Маркова выполнены, случайные ошибки нормально распределены, уровень доверия равен $\Sexpr{ci_level * 100}$\%, критическое значение $t$-статистики равно $\Sexpr{t_crit}$, всего $n$ наблюдений. Регрессия имеет вид $\hat Y_i = \underset{(3)}{-4} + \underset{(0.2)}{5}X_i$, в скобках указаны стандартные ошибки. Доверительный интервал для $\beta_2$ равен
 \begin{multicols}{3} % располагаем ответы в 3 колонки
   \begin{choices} % опция [o] не рандомизирует порядок ответов
      \correctchoice{$[\Sexpr{round(5 - 0.2 * t_crit, 2)}; \Sexpr{round(5 + 0.2 * t_crit, 2)}]$}
      \wrongchoice{$[\Sexpr{5 -  t_crit}; \Sexpr{5 +  t_crit}]$}
      \wrongchoice{$[\Sexpr{5 - 2 * t_crit}; \Sexpr{5 + 2 * t_crit}]$}
      \wrongchoice{$[\Sexpr{round(5 - 0.2/sqrt(n) * t_crit, 2)}; \Sexpr{round(5 + 0.2/sqrt(n) * t_crit, 2)}]$}
      \wrongchoice{$[\Sexpr{round(5 - 0.2 * sqrt(n) * t_crit, 2)}; \Sexpr{round(5 + 0.2 * sqrt(n) * t_crit, 2)}]$}
...
\element{combat_kr_01_16}{ % в фигурных скобках название группы вопросов
 %\AMCnoCompleteMulti
  \begin{questionmult}{pr_06} % тип вопроса (questionmult — множественный выбор) и в фигурных — номер вопроса
    В парной регрессии величина \Sexpr{zero_condition}
 \begin{multicols}{2} % располагаем ответы в 3 колонки
   \begin{choices} % опция [o] не рандомизирует порядок ответов
...
<<echo=FALSE>>=
r2_all <- c(0.9, 0.8, 0.7, 0.6)
TSS_all <- c(100, 200, 300, 400)
n <- 12
k <- 2

set.seed(var_no)
r2 <- sample(r2_all, 1)
TSS <- sample(TSS_all, 1)
ESS <- TSS * r2
RSS <- TSS - ESS
hat_s2 <- round(RSS / (n - k), 2)
@

\element{combat_kr_01_16}{ % в фигурных скобках название группы вопросов
 %\AMCnoCompleteMulti
  \begin{questionmult}{pr_08} % тип вопроса (questionmult — множественный выбор) и в фигурных — номер вопроса
    В модели парной регрессии $R^2 = \Sexpr{r2}$, $TSS = \Sexpr{TSS}$ и $\Sexpr{n}$ наблюдений. Несмещённая оценка дисперсии случайной ошибки равна
 \begin{multicols}{3} % располагаем ответы в 3 колонки
   \begin{choices} % опция [o] не рандомизирует порядок ответов
      \correctchoice{$\Sexpr{hat_s2}$}
      \wrongchoice{$\Sexpr{hat_s2 + 0.1}$}
      \wrongchoice{$\Sexpr{hat_s2 + 0.2}$}
      \wrongchoice{$\Sexpr{hat_s2 - 0.2}$}
      \wrongchoice{$\Sexpr{hat_s2 - 0.1}$}
      \wrongchoice{$\Sexpr{hat_s2 + 0.3}$}
   \end{choices}
  \end{multicols}
  \end{questionmult}
}
...
<<echo=FALSE>>=
multiplier_text_all <- c("два раза", "три раза", "четыре раза")
multiplier_all <- 2:4

set.seed(var_no)
i <- sample(length(multiplier_all), 1)
multiplier_text <- multiplier_text_all[i]
multiplier <- multiplier_all[i]
@
...
\element{combat_kr_01_16}{ % в фигурных скобках название группы вопросов
 %\AMCnoCompleteMulti
  \begin{questionmult}{pr_09} % тип вопроса (questionmult — множественный выбор) и в фигурных — номер вопроса
    Если все $Y_i$ в линейной регрессии увеличить в \Sexpr{multiplier_text}, то оценка $\hat\beta_2$
 \begin{multicols}{2} % располагаем ответы в 3 колонки
   \begin{choices} % опция [o] не рандомизирует порядок ответов
      \correctchoice{помножится на $\Sexpr{multiplier}$}
      \wrongchoice{помножится на $\Sexpr{2 * multiplier}$}
      \wrongchoice{поделится на $\Sexpr{multiplier}$}
      \wrongchoice{поделится на $\Sexpr{2 * multiplier}$}
...
<<echo=FALSE>>=
set.seed(var_no)
alpha <- sample(c(0.05, 0.1, 0.01, 0.07), 1)
p_value <- alpha - 0.01
@
...
\element{combat_kr_01_16}{ % в фигурных скобках название группы вопросов
 %\AMCnoCompleteMulti
  \begin{questionmult}{pr_10} % тип вопроса (questionmult — множественный выбор) и в фигурных — номер вопроса
   Если $\alpha = \Sexpr{alpha}$ и $P$-значение равно $\Sexpr{p_value}$, то
 \begin{multicols}{2} % располагаем ответы в 3 колонки
   \begin{choices} % опция [o] не рандомизирует порядок ответов
