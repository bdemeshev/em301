...
Информационная матрица (матрица Гессе с минусом):
\[\widehat{I} = - \frac{\partial^2 l}{\partial \theta^2} \left(\theta \right)\]
...
При верной \(H_0\):
\[W = \left(g\left(\hat{\theta}_{UR} \right) \right)' \cdot \left( \left(\dfrac{\partial l}{\partial \theta}\left(\hat{\theta}_{UR} \right)\right) \cdot \widehat{I}^{-1} \left(\hat{\theta}_{UR} \right) \cdot \left(\dfrac{\partial l}{\partial \theta}\left(\hat{\theta}_{UR} \right)\right)' \right)^{-1} \cdot g\left(\hat{\theta}_{UR} \right) \sim \chi^2_q \]
