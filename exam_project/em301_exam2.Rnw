...
\title{Экзамен по эконометрике 2014. Демо-версия 2.}

\begin{document}
%\maketitle
\parindent=0 pt % no indent

Экзамен по эконометрике 2014. Версия 2.
...
\item Найдите TSS, ESS, RSS.
\item Найдите несмещенную оценку дисперсии МНК-коэффициента $\hb_3$.
\item Найдите $\widehat{Var}(\hb_1 - \hb_2 + 3\hb_3 - 5\hb_4)$
\end{enumerate}
...
\item Показать, какие векторы перпендикулярны.
\item Показать на графике TSS, ESS и RSS.
\item Объяснить геометрический смысл МНК.
\item На основе предыдущих пунктов вывести $\hb$ в матричном виде.
\end{enumerate}
...
\item Выпишите предположение о нормальности остатков и объясните, что оно даёт.
\item Обозначьте, к чему приводит нарушение предпосылки теоремы о ранге матрицы $X_{n \times k}$. Назовите это понятие, дайте ему определение и опишите свойства оценок в данном случае.
\end{enumerate}

\item Гетероскедастичность.

\begin{enumerate}
\item Подробно опишите последствия гетескедастичности.
\item Укажите три способа борьбы с гетероскедастичностью и поясните, как они решают проблему (такие способы борьбы, как <<ничего не делать>>, не принимаются). Каждый из методов должен иметь краткую характеристику и обоснование, как он решает проблему.
\item По графикам определите, есть ли гетероскедастичность. И если есть, то объясните, по каким признакам Вы это поняли.

\begin{center}
<<out.height = "6cm", out.width = "12cm">>=

a <- ggplot(flats, aes(x = totsp, y = price)) + geom_point() + 
     labs(x="x", y="y")

b <- ggplot(flats, aes(x = dist, y = price)) + geom_point() + 
     labs(x="x", y="y")
...
\begin{enumerate}
\item Приведите свойства стационарного временного ряда.
\item По графику определите, стационарный ряд или нет. Свой вывод обоснуйте.

\begin{center}
<<out.height = "7cm", out.width = "9cm", message=FALSE>>=
g <- getSymbols(Symbols = "AAPL", from = "2012-11-01", to = "2013-02-01")
y <- AAPL$AAPL.Adjusted
plot(y, main ="", xlab="t", ylab="y")
@
\end{center}

\item Приведите пример MA(3).
\item Приведите определения ACF, PACF.
...
@
\item Выпишите функцию максимального правдоподобия для AR(2).
\end{enumerate}
