...
\begin{problem}
Данные описываются моделью $y_t = \beta + \varepsilon_t$, где $\varepsilon_t$ --- стационарный AR(1) процесс, $\varepsilon_t=u_t+\rho \varepsilon_{t-1}$, $u\sim N(0;\sigma^2\cdot I)$. Имеются наблюдения $y'=(1, 2, 0, 0, 1)$. 
\begin{enumerate}
