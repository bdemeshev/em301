...
\begin{problem}
Найдите безусловная дисперсия GARCH-процессов 
\begin{enumerate}
...
\begin{enumerate}
  \item Выпишите функцию правдоподобия $\mathrm{L}(\mu, \rho, \sigma^2) = f_{Y_1}(y_1)\prod_{t=2}^{T}f_{Y_t|Y_{t-1}}(y_t|y_{t-1})$.
  \item Максимизируя условную функцию правдоподобия $\mathrm{L}(\mu, \rho, \sigma^2|Y_1 = y_1) = \prod_{t=2}^{T}f_{Y_t|Y_{t-1}}(y_t|y_{t-1})$, найдите оценки неизвестных параметров модели.
\end{enumerate}
\end{problem}

\begin{solution}
Поскольку имеют место соотношения $\varepsilon_1 = \rho \varepsilon_0 + u_1$ и $Y_1 =\mu + \varepsilon_1$, то из условия задачи получаем, что $\varepsilon_1 \sim N(0,\sigma^2 / (1 - \rho^2))$
