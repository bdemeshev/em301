...
Показатель & Значение \\
\midrule
$R^2$                        & \Sexpr{round(report$r.squared, 3)} \\
...
Остаток     & \textbf{B1}  &  \Sexpr{round(deviance(model), 1)}  &              &       \\
...
\begin{tabular}{rrrrr}
 ...
 & Оценка & Ст. ошибка & t-статистика  \\
  \hline
Константа & \Sexpr{ct[1, 1]} & \Sexpr{ct[1, 2]} & \Sexpr{ct[1, 3]} \\
  $exper$ & \textbf{В8} & \Sexpr{ct[2, 2]} & \Sexpr{ct[2, 3]}  \\
  $exper^2$ & \Sexpr{ct[3, 1]} & \Sexpr{ct[3, 2]} & \Sexpr{ct[3, 3]}  \\
  $sex$ & \Sexpr{ct[4, 1]} & \Sexpr{ct[4, 2]} & \textbf{В10}  \\
   \hline
\end{tabular}
\end{table}
...
\[
\hat Y_i = 2.2 + 0.104 X_i - 3.48 Z_i + 0.34 W_i, \; ESS = \Sexpr{ESS_old}, \; RSS = 20
\]
